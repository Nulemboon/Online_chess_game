...
std::unordered_map<std::string, int> session; //map from username to socket
std::unordered_map<int, std::string> getName; //map from socket to username
    ...
    switch (status) {
        case 1:
            ...
            *username = acc->getUsername(); // Assign the username of this session
            ...
            getName.insert({clientSocket, *username});
...
void logout(int clientSocket) {
    std::string username = getName[clientSocket];

    getName.erase(clientSocket);
...
void invite(int clientSocket, Message* msg, Database* db) {
    ...
    std::string curName = getName[clientSocket];
...
void move(int clientSock, Message *msg, Database* db) {
